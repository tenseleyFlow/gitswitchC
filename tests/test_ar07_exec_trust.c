...
TEST(extended_acl_mutation_or_nontriviality_is_rejected) {
    ...
    if (acl_rc != 0) {
        printf("  INFO: filesystem does not support a nontrivial ACL fixture\n");
        return;
    }
#else
...
#if defined(__FreeBSD__)
    if (acl_rc != 0) {
        printf("  INFO: filesystem cannot apply the leaf ACL fixture\n");
        return;
    }
#else
    CHECK_EQ_INT(acl_rc, 0);
...
TEST(nonregular_leaf_is_bounded_and_execute_only_policy_is_explicit) {
    char root[MAX_PATH_LEN], bin[MAX_PATH_LEN];
    ...
    if (getuid() != (uid_t)0) {
        ...
        /* The conservative format policy requires read+execute permission:
         * an unreadable file might be a script with an unvalidated shebang. */
